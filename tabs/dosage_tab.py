    ...
    if dosage_input and calib_params:
        ...
        if dosage_results is not None and not dosage_results.empty:
            ...
            try:
                ...
                fig_dos = px.scatter(dosage_results_sorted, x='Masse_Adsorbant_g', y='qe', title="Effect of Adsorbent Mass on qe", labels={'Masse_Adsorbant_g': "Adsorbent Mass (g)", 'qe': 'qe (mg/g)'}, hover_data=dosage_results_sorted.columns)
                fig_dos.add_trace(go.Scatter(x=dosage_results_sorted['Masse_Adsorbant_g'], y=dosage_results_sorted['qe'], mode='lines', name="Trend", showlegend=False))
                ...
                try:
                    ...
                    fig_dos_styled.add_trace(go.Scatter(x=df_dos_styled_dl['Masse_Adsorbant_g'],y=df_dos_styled_dl['qe'],mode='markers+lines',marker=dict(symbol='square', color='black', size=10),line=dict(color='red', width=3),name="Experimental data"))
