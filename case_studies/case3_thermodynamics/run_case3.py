...
for T_C in temperatures:
    ...
    popt, pcov = curve_fit(
        langmuir, Ce, qe, p0=[80, 0.02], bounds=([0, 0], [500, 10]), maxfev=20_000
    )
    ...
    print(
        f"    {T_C} °C ({T_C + 273.15:.2f} K):  qm = {popt[0]:.2f} mg/g,  "
...
for i, T_C in enumerate(temperatures):
    ...
    ax1.plot(
        Ce_fine,
        langmuir(Ce_fine, fit["qm"], fit["KL"]),
        "-",
        color=colors[i],
        lw=2,
...
ax3.fill_between(
    temperatures, min(delta_G) * 1.3, 0, alpha=0.08, color="green", label="Spontaneous region"
)
...
pd.DataFrame(lang_rows).to_csv(
    OUT_DIR / "langmuir_per_temperature.csv", index=False, float_format="%.5g"
)
...
thermo_df.to_csv(OUT_DIR / "thermodynamic_parameters.csv", index=False, float_format="%.5g")
...
json_out = {
    ...
    "thermodynamics": {
        ...
        "delta_G_kJ_mol": {str(T_C): float(dG) for T_C, dG in zip(temperatures, delta_G)},
        "mechanism": mechanism,
    },
}
